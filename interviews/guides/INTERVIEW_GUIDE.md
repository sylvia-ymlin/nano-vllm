...
> "我开发了 Nano-vLLM，一个从零实现的轻量级 LLM 推理引擎。项目在性能上超过 vLLM 5.3%（1434 vs 1362 tokens/s），同时保持代码简洁性（仅 ~1200 行 Python）。核心实现包括高效的调度引擎、前缀缓存优化、CUDA 自定义核、以及多 GPU 张量并行支持。这个项目展示了我在系统设计、GPU 编程和性能优化方面的能力。"
